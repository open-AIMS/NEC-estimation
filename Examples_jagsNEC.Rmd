---
title: "NEC-estimation examples - fit.jagsNEC"
...
knitr::opts_chunk$set(echo = TRUE, include=T, message = TRUE, warnings = FALSE, warn.conflicts = FALSE, cache=T)
```

## Background

This document contains examples for running the NEC-estimation package 'jagsNEC' currently under development. The package is being developed at  <https://github.com/AIMS/NEC-estimation> and is a No-Effect-Concentration (NEC) model specific wrapper for the R2jags package available in R. The NEC mode fit is based on the model described in [@Fox2010], although we have generalised the package to allow a large range of response variables to be modelled using the appropriate statistical distribution. We have further added a range of alternative NEC model types, as well as a range of typically used concentration-response models (such as 4-parameter logistic and weibull models) that have no NEC 'step' function but simply models response as a smooth function of concentration. Using the function fit.jagsMANEC it is possible to fit a specific set or all of the available models, and thereby obtain all the individual model fits, or a model weighted estimate of predicted posterior values, based on DIC model weights.

An additional endpoint has also been derived using Bayesian posterior predicted values to estimate the "No-Statistical-Effect-Concentration" as the concentration at which predicted values for each MCMC chain fall below a lower percentile bound (defined as sig.val) of the control. NSEC estimates are currently used to approximate NEC for models without a specific NEC step parameter (in jagsNEC these have the prefix ECx in their model name).
