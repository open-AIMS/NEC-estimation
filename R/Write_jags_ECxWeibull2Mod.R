...
write.jags.ECxWeibull2.mod <- function(x="gamma", y, mod.dat){  
  ...
  # binomial y; gamma x ----
   if(x=="gamma" & y=="binomial"){
    ...
    cat("
        ...
        for (i in 1:N)
        {
        theta[i]<-top + (bot−top) * (1−exp(−exp(beta*(log(x[i]) − log(EC50))))) 
 ...
 if(x=="gamma" & y=="gaussian"){
  ...
  if(x=="gaussian" & y=="negbin"){
    ...
    cat("
        ...
        for (i in 1:N)
        {
        theta[i]<-size/(size+ top + (bot−top) * (1−exp(−exp(beta*(log(x[i]) − log(EC50))))) )
