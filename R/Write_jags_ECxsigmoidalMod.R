...
#' write.jags.ECxsigmoidal.mod
#'
#' Writes an ECxsigmoidal model file and generates a function for initial values to pass to jags
#'
...
write.jags.ECxsigmoidal.mod <- function(x = "gamma", y, mod.dat) {
  ...
  if (x == "gamma" & y == "gaussian") {
    ...
    cat("
      ...
      for (i in 1:N)
      {
      theta[i]<-top*exp(-beta*(x[i])^d)-alpha # extra parameter alpha is offset to y
  ...
  if (x == "gaussian" & y == "negbin") {
    ...
    cat("
        ...
        for (i in 1:N)
        {
        theta[i]<-size/(size+top*exp(-beta*(x[i])^d)
